...
def plot_isochrone_map(reconstructed_gdf, fragmented_gdf, utah_boundary, places, source_h3, thresholds):
    fig, ax = plt.subplots(figsize=(8, 8))
    utah_boundary.plot(ax=ax, color="lightgrey", edgecolor="black", linewidth=0.5)

    # Discrete colormap with N = bins
    # cmap = plt.get_cmap("YlOrRd", len(thresholds) + 1)
    cmap = plt.get_cmap("RdYlGn_r", len(thresholds) + 1)

    # 1) Reconstructed rings (background)
    if not reconstructed_gdf.empty:
        reconstructed_gdf.plot(
            ax=ax,
            column="duration_bin",
            cmap=cmap,
            linewidth=0.05,
    ...
    # 2) Fragmented observed cells (foreground, darker)
    if not fragmented_gdf.empty:
        fragmented_gdf.plot(
            ax=ax,
            column="duration_bin",
            cmap=cmap,
    ...
    gpd.GeoSeries([source_poly], crs="EPSG:4326").plot(ax=ax, color="blue", alpha=0.9, label="Source")
...
def main():
    ...
    plot_isochrone_map(rings_gdf, fragmented_gdf, utah_boundary, places, source_h3, THRESHOLDS)
