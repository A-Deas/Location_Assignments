...
# default chunk size: how many files per Slurm array task
...
def compute_bin_to_k_for_source(source_h3, con, thresholds):
    ...
    # observed=False to keep pandas quiet and stable across versions
...
def assign_location_with_drivetimes(
    ...
    if not bin_to_k:
        # No drivetime info at all → we’ll fall back to legacy later
        bins = [0] + thresholds
        labels = [f"{bins[i]}–{bins[i+1]}" for i in range(len(bins) - 1)]
    ...
    # 2) Fallback if *no* amenity-matching hex was found in any ring
    if not found_match:
        no_match_counter += 1
        if bin_to_k:
...
def process_population_file(
    ...
    """
    Reads one parquet file, fills h3_act where missing using drivetime-informed rings,
    and adds a 'drivetime_sec' column.
    ...
    df["h3_act"] = df["h3_act"].astype("object")
    ...
    for pid, group_idx in df.groupby("p_id", observed=False).groups.items():
        ...
        for row_idx in idxs:
            ...
            # Case 1: already have a location
            ...
            # Case 2: missing h3_act → need to assign
            else:
                chosen_hex = None
                duration_sec = np.nan
                ...
                if source_hex is not None and source_hex in available_sources_set:
                    ...
                    # Legacy fallback: small disk around source_hex using amenities
                ...
                # Final safety fallback: if still none, stay at source or home
        ...
        print(f"Finished with agent: {pid} in file {file_path.name}")
...
def main():
    ...
    all_files = sorted(POP_FOLDER.glob("*.parquet"))

    if not all_files:
        print("No parquet files found in Synthetic Population folder.")
        return

    # Figure out which chunk this task should process
    task_id_str = os.environ.get("SLURM_ARRAY_TASK_ID", None)
    if task_id_str is None:
        # Local / debugging mode: just process first CHUNK_SIZE files
        task_id = 0
        print(f"[Local mode] No SLURM_ARRAY_TASK_ID; defaulting to task_id=0, CHUNK_SIZE={CHUNK_SIZE}")
    else:
        task_id = int(task_id_str)
        print(f"[HPC mode] SLURM_ARRAY_TASK_ID={task_id}, CHUNK_SIZE={CHUNK_SIZE}")

    start_idx = task_id * CHUNK_SIZE
    end_idx = min(start_idx + CHUNK_SIZE, len(all_files))

    if start_idx >= len(all_files):
        print(f"Task {task_id}: start_idx {start_idx} >= number of files {len(all_files)}. Nothing to do.")
        return

    files_for_this_task = all_files[start_idx:end_idx]
    print(f"Task {task_id}: processing files {start_idx} to {end_idx - 1}")
    for fp in files_for_this_task:
        print(f"  - {fp.name}")

    global_no_match = 0
    global_legacy_fallback = 0

    for file_path in files_for_this_task:
    ...
    print(f"Files processed: {len(files_for_this_task)}")
